...
import { Sparkles, Github, Play, Loader2, GitCommit, History, ChevronRight, X, Clock, CheckCircle2, AlertTriangle, ArrowUp } from 'lucide-react';
...
export default function SecurityAuditApp() {
  ...
  const startAudit = (urlOverride?: string) => {
    const urlToUse = urlOverride || repoUrl;
    if (!urlToUse) return;
  ...
  if (view === 'landing') {
    return <LandingPage onRoast={(url) => {
      setRepoUrl(url);
      setView('app');
      // Small timeout to allow render to switch view before starting audit
      setTimeout(() => startAudit(url), 100);
    }} />;
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0B0F14] text-[#E6EEF8] font-sans overflow-hidden">
      {/* Top Bar */}
      <header className="h-16 border-b border-[#1C2430] flex items-center justify-between px-6 bg-[#0B0F14] z-50 shrink-0">
        <div 
        ...
        >
          <img 
            ...
            alt="Roastybara Logo" 
            className="w-8 h-8 object-contain transform rotate-3 transition-transform group-hover:rotate-6" 
          />
          <h1 className="text-xl font-display font-bold tracking-tight text-[#E6EEF8]">Roastybara</h1>
        </div>
        ...
        <div className="flex-1 max-w-2xl mx-8">
          <div className="relative group">
            ...
            {/* Dynamic Tooltip */}
            {status === 'idle' && showTooltip && !repoUrl && (
              <div 
                className="absolute top-full left-0 mt-3 z-50 animate-bounce-slight cursor-pointer"
                onClick={() => setShowTooltip(false)}
              >
                <div className="bg-[#4DA3FF] text-white text-xs font-medium px-3 py-2 rounded-lg shadow-lg relative whitespace-nowrap hover:bg-[#3b82f6] transition-colors">
                  <div className="absolute -top-1 left-4 w-2 h-2 bg-[#4DA3FF] transform rotate-45"></div>
                  <div className="flex items-center gap-2">
                    <span>👈 Start here! Paste a repo URL to unleash the agents.</span>
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
          ...
          <div className="flex-none h-[40%] min-h-[220px] p-6 border-b border-[#1C2430] flex flex-col bg-[#0B0F14] z-10 relative">
            ...
            <div className="mt-2 text-center shrink-0">
               <p className="text-[10px] text-[#8FA3B8]/50 uppercase tracking-widest font-medium">
                 {status === 'idle' 
                   ? 'Timeline will populate during audit' 
                   : 'Interactive Timeline • Select any commit to inspect'}
               </p>
            </div>
          </div>
